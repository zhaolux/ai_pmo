...
from .weekly_report import build_weekly_reports
...
def execute(command: str, as_of: date | None = None) -> int:
    paths = get_paths()
    ...
    try:
        ...
        if command == "decision-brief":
            ...
            json_file, docx_file = build_decision_brief(paths.suite, mapping, report_date, agent_report)
...
def main() -> None:
    ...
    parser.add_argument("command", choices=["init", "seed", "scan", "validate", "build", "run", "agents", "weekly-report", "decision-brief"])
    parser.add_argument("--date", type=date.fromisoformat, default=None, help="Agent分析的数据日期，格式YYYY-MM-DD")
    args = parser.parse_args()
    raise SystemExit(execute(args.command, args.date))
