        ...
        for item in milestones:
            connection.execute(
                ...
                (item["milestone_id"], project["project_id"], item["phase_id"],
                 item["milestone_name"], item["planned_date"], item["deliverable"],
                 item["exit_criteria"],
                 "计划日期已到，待确认" if item["planned_date"] <= "2026-09-16" else "未开始"),
            )
...
def validate_planning(paths: Paths) -> list[str]:
    ...
    with connect(paths.database) as connection:
        ...
        for row in connection.execute(
            "SELECT milestone_id,phase_id,planned_date FROM baseline_milestone"
        ):
            start, finish = phase_bounds[row["phase_id"]]
            if row["planned_date"] < start or (finish and row["planned_date"] > finish):
